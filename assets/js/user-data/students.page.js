$(document).ready(function() {
    // alert("Load!")
    // Load Data to table
    getStudents();

});

$("#btn_update").click(function (e) {      
    e.preventDefault();
    // alert("Clicked!")
    updateStudents()
});


function ajaxCall(params = {}) {
    return new Promise((resolve, reject) => {
      $.ajax({
        url: params.url,
        method: params.method,
        data: params.data,
        dataType: "json",
        success: function (response) {
          resolve(response);
        },
        error: function (error) {
          reject(error);
        },
      });
    });
  }
  
    async function getStudents() {
      
        try {
        let list = await ajaxCall({
            data: "&action=get&request=students",
            method: "post",
            url: "../../app/routers/AdminRouter.php",
        });
        
        //   console.log(JSON.stringify(list));
        
            var tableRows = '';
            // Loop through data and create table rows
            $.each(list.result.result_list, function(index, item) {
                tableRows += '<tr>';
                tableRows += '<td>' + item.spn + '</td>';
                tableRows += '<td>' + item.firstname + " " + item.middlename + " " + item.lastname + '</td>';
                tableRows += '<td>' + item.gender + '</td>';
                tableRows += '<td>' + item.level + '</td>';
                tableRows += '<td>' + item.section + '</td>';
                tableRows += '<td>' + item.major + '</td>';
                tableRows += '<td>' + item.date_enrolled + '</td>';
                tableRows += '<td>' + item.schoolyear + '</td>';
                // tableRows += '<td>';
                // tableRows += '<button type="button" class="btn btn-info rounded-pill" data-id="' + item.uid + '"><i class="bi bi-info-circle"></i>&nbspUpdate</button> ';
                // tableRows += '<button type="button" class="btn btn-danger rounded-pill" data-id="' + item.uid + '"><i class="bi bi-exclamation-octagon"></i>&nbspRemove</button> ';
                // tableRows += '</td>';
                tableRows += '</tr>';
            });

            $('#tbl_students tbody').html(tableRows); // Insert rows into table body

            Swal.fire({
                toast: true,
                position: "bottom-end",
                icon: "success",
                title: "Students loaded!",
                showConfirmButton: false,
                timer: 3000,
            });
                            
    
        } catch (e) {
        
        Swal.fire({
            title: "Error",
            text: e.message,
            icon: "error",
            confirmButtonText: "Try again",
        });
    
        }
    }


    async function updateStudents() {
      
        try {
        let list = await ajaxCall({
            data: "&action=update&request=students",
            method: "post",
            url: "../../app/routers/AdminRouter.php",
        });
        
          console.log(JSON.stringify(list));

            Swal.fire({
                toast: true,
                position: "bottom-end",
                icon: "success",
                title: "Students List Updated!",
                showConfirmButton: false,
                timer: 3000,
            });
                            
    
        } catch (e) {
        
        Swal.fire({
            title: "Error",
            text: JSON.stringify(e.message),
            icon: "error",
            confirmButtonText: "Try again",
        });
    
        }
    }

   
